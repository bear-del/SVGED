...
data_t = LoadData_pickle(path='./data/AUV/',name='C0_test')
label = 'ca'
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch == epochs - 1:
            for class_num in range(8):
                data_T_for_gen = LoadData_pickle(path='./data/AUV/', name=f'C{class_num}_train')
                # data_T_for_gen = scaler.transform(data_T_for_gen)
                # file1 = np.squeeze(file1)
                # gentime = datetime.strftime(datetime.now(), '%m%d %H%M%S')
                data_T_encoded = encoder.predict(data_T_for_gen, verbose=1)
                data_T_encoded = np.array(data_T_encoded)
                print(data_T_encoded.shape)
                file_T = open('./data/AUV/type9/C' + str(class_num) + '_train'+label+'.pkl', 'wb')
                pickle.dump(data_T_encoded, file_T)
                file_T.close()

                data_t_for_gen = LoadData_pickle(path='./data/AUV/', name=f'C{class_num}_test')
                # data_t_for_gen = scaler.transform(data_t_for_gen)
                data_t_encoded = encoder.predict(data_t_for_gen, verbose=1)
                data_t_encoded = np.array(data_t_encoded)
                print(data_t_encoded.shape)
                file_t = open('./data/AUV/type9/C' + str(class_num) + '_test'+label+'.pkl', 'wb')
                pickle.dump(data_t_encoded, file_t)
                file_t.close()
                #
                # data_TZ_encoded = encoder.predict(np.random.randn(840, 3072), verbose=1)
                # data_TZ_encoded = np.array(data_TZ_encoded)
                # print(data_TZ_encoded.shape)
                # file_TZ = open('./data/AUV/type9/C0_G_train_Z'+label+'.pkl', 'wb')
                # pickle.dump(data_TZ_encoded, file_TZ)
                # file_TZ.close()
                #
                # data_tZ_encoded = encoder.predict(np.random.randn(360, 3072), verbose=1)
                # data_tZ_encoded = np.array(data_tZ_encoded)
                # print(data_tZ_encoded.shape)
                # file_tZ = open('./data/AUV/type9/C0_G_test_Z'+label+'.pkl', 'wb')
                # pickle.dump(data_tZ_encoded, file_tZ)
                # file_tZ.close()
