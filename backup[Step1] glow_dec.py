    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch == epochs - 1:
            for class_num in range(8):
                ...
                file_T = open('./data/AUV/type9/C' + str(class_num) + '_G_train.pkl', 'wb')
                ...
                data_t_decoded = np.array(data_t_decoded)
                print(data_t_decoded.shape)
                file_t = open('./data/AUV/type9/C' + str(class_num) + '_G_test.pkl', 'wb')
                pickle.dump(data_t_decoded, file_t)
                file_t.close()

                data_TZ_decoded = decoder.predict(np.random.randn(840, 3072), verbose=1)
                data_TZ_decoded = np.array(data_TZ_decoded)
                print(data_TZ_decoded.shape)
                file_TZ = open('./data/AUV/type9/C0_G_train_Z.pkl', 'wb')
                pickle.dump(data_TZ_decoded, file_TZ)
                file_TZ.close()

                data_tZ_decoded = decoder.predict(np.random.randn(360, 3072), verbose=1)
                data_tZ_decoded = np.array(data_tZ_decoded)
                print(data_tZ_decoded.shape)
                file_tZ = open('./data/AUV/type9/C0_G_test_Z.pkl', 'wb')
                pickle.dump(data_tZ_decoded, file_tZ)
                file_tZ.close()
