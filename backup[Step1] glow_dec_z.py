...
data_T_decoded = decoder.predict(np.random.randn(840, 3072), verbose=1)
data_T_decoded = np.array(data_T_decoded)
print(data_T_decoded.shape)
file_T = open('./data/AUV/type9/C0_G_train_Z.pkl', 'wb')
...
data_t_decoded = decoder.predict(np.random.randn(360, 3072), verbose=1)
data_t_decoded = np.array(data_t_decoded)
print(data_t_decoded.shape)
file_t = open('./data/AUV/type9/C0_G_test_Z.pkl', 'wb')
pickle.dump(data_t_decoded, file_t)
file_t.close()
