...
# faults= ['x0','x1','x2']
# root = './data/AUV/type8_label/'
root = './data/AUV/type9_label/'
for fault in faults :
    data_dir = root+fault+'_train'+label+'.pkl'
    with open(data_dir, 'rb') as f:
        datasets,labels= pickle.load(f)
    dataset_train= nextbatch.DataProvider(datasets, labels) #绑定

    test_dir = root+fault+'_test'+label+'.pkl'
...
ckpt_path = './model/dcae-' + fault + label + '/model.ckpt'
...
np.savetxt('./model/dcae-' + fault + + label +'/dcae_loss_'+fault+'.txt',af)
plt.plot(af, label='train loss')
plt.savefig('./model/dcae-' + fault + + label +'/loss_dcae.png')
plt.show()
